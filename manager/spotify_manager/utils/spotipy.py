...
os.environ["SPOTIPY_CLIENT_ID"] = "63fa0be7f0c34299958e2b89ba0121b6"
os.environ["SPOTIPY_CLIENT_SECRET"] = "92ee9c61dd78498789a481e8f02ce4c5"
os.environ["SPOTIPY_REDIRECT_URI"] = "http://localhost:8869/callback"
...
def run(width=825, height=510):
    ...
    # Set up events
    root.bind(
        "<Button-1>", lambda event: mouse_wrapper(left_pressed, event, canvas, data)
    )
    root.bind(
        "<Button-3>", lambda event: mouse_wrapper(right_pressed, event, canvas, data)
    )
    canvas.bind(
        "<Motion>", lambda event: mouse_wrapper(mouse_moved, event, canvas, data)
    )
    canvas.bind(
        "<B1-Motion>", lambda event: mouse_wrapper(left_moved, event, canvas, data)
    )
    canvas.bind(
        "<B3-Motion>", lambda event: mouse_wrapper(right_moved, event, canvas, data)
    )
    root.bind(
        "<B1-ButtonRelease>",
    ...
    root.bind("<KeyPress>", lambda event: key_wrapper(key_pressed, event, canvas, data))
    root.bind(
        "<KeyRelease>", lambda event: key_wrapper(key_released, event, canvas, data)
    )
...
class Button(object):
    ...
    def draw(self, canvas, data):
        ...
        canvas.create_rectangle(
            self.x1, self.y1, self.x2, self.y2, fill=self.fill, width=0
        )
        canvas.create_text(
            self.cx,
            self.cy,
...
def help_redraw_all(canvas, data):
    canvas.create_rectangle(0, 0, data.width, data.height, fill=black_background)
    canvas.create_rectangle(
        data.width * (1 / 2 - scale / 5),
        0,
        data.width * (1 / 2 + scale / 5),
        data.height,
        fill=black,
    ...
    canvas.create_image(data.width / 2, data.height / (3 * scale), image=data.logo)
    canvas.create_text(
        data.width / 2,
        5 * data.height / scale ** 6,
        fill=gray,
        text="Simplify. Organize. Analyze. ",
        font="Proxima 14",
    )
    canvas.create_line(
    ...
    canvas.create_text(
        data.width / 2,
        data.height * scale * 6 / 24,
        text="Maximize the window for optimal viewing.",
        fill=white,
        font="Proxima 14 bold",
    )
    canvas.create_text(
        data.width / 2,
        data.height * scale * 7.25 / 24,
    ...
    canvas.create_text(
        data.width / 2,
        data.height * scale * 13 / 24,
        text="Use buttons or hold control + drag to add to the playlist.",
        fill=white,
        font="Proxima 14 bold",
    )
    canvas.create_text(
        data.width / 2,
        data.height * (1 - 1 / scale ** 6),
...
def login_buttons(data):
    return [
        ...
        Button(
            ...
            authorize,
            color=green,
            color_hover=green_hover,
            text_size=14,
        ),
        Button(
            data,
            "PRESS THE SPACEBAR FOR HELP AT ANY TIME",
            1 / 2,
            1 - 1 / scale ** 6,
            help_init,
            width_scale=1 / 2,
...
def login_mouse_moved(data):
    for button in data.buttons:
        if button.is_within_bounds(data, data.mouse_moved_x, data.mouse_moved_y):
            button.hover()
        else:
            button.unhover()
...
def login_left_released(data):
    for button in data.buttons:
        if button.is_within_bounds(data, data.left_released_x, data.left_released_y):
            button.press(data)
...
def login_redraw_all(canvas, data):
    ...
    canvas.create_text(
        ...
        5 * data.height / scale ** 6,
        fill=gray,
        text="Simplify. Analyze. Organize. ",
        font="Proxima 14",
    )
    canvas.create_line(
        data.width * (1 / 2 - 1 / scale ** 3),
    ...
    for term in data.artists_top:
        ...
        for artist in artists["items"]:
            ...
            if artist["id"] not in data.artists:
                data.artists[artist["id"]] = {"name": artist["name"], "songs": list()}
...
def get_playlists(data):
    ...
    for playlist in playlists["items"]:
        data.playlists[playlist["id"]] = {"name": playlist["name"], "songs": list()}
        uri = playlist["uri"]
...
class Sidebar(Button):
    ...
    def draw(self, canvas, data):
        ...
        if self.is_selected(data):
            canvas.create_rectangle(
                0, self.y1, data.width / data.columns / 9, self.y2, fill=green_hover
            )
...
class Artist(Button):
    ...
    def draw(self, canvas, data):
        ...
        canvas.create_rectangle(
            self.x1 + data.width / data.columns / 3,
            self.y1,
        ...
        canvas.create_text(
            ...
            self.cy,
            text=truncate(self.artist_name, data.width / data.columns * 4),
            fill=self.text_fill,
            anchor=W,
            font="Proxima 10",
        )
        if not self.is_all_staged(data):
            canvas.create_text(
                self.x1 + data.width / data.columns / 2,
                self.cy,
                text="+",
                fill=self.plus_fill,
...
class Song(Button):
    def __init__(self, data, row, song_id):
        ...
        self.duration = "%d:%02d" % (duration_ms // 60000, duration_ms % 60000 // 1000)
        super().__init__(
            data,
            text,
            cx_scale,
    ...
    def draw(self, canvas, data):
        ...
        canvas.create_text(
            self.x1 + data.width / data.columns * 0.75,
            self.cy,
            text=truncate(self.title, data.width / data.columns * 4),
            fill=self.text_fill,
            anchor=W,
            font="Proxima 10",
        )
        canvas.create_text(
            data.width * 7.75 / data.columns,
            self.cy,
            text=truncate(self.artist_name, data.width / data.columns * (3.25)),
            fill=self.text_fill,
            anchor=W,
            font="Proxima 10",
        )
        canvas.create_text(
            data.width * 11 / data.columns,
            self.cy,
            text=truncate(self.album_name, data.width / data.columns * (4)),
            fill=self.text_fill,
            anchor=W,
            font="Proxima 10",
        )
        canvas.create_text(
            data.width * 15 / data.columns,
            self.cy,
            text=self.duration,
            fill=gray,
            anchor=W,
    ...
    def draw(self, canvas, data):
        if self.row < 40:
            ...
            canvas.create_text(
                ...
                text=truncate(
                    ("%d. %s | %s")
                    % (self.row + data.start_stage, self.title, self.artist_name),
                    self.width,
                ),
                fill=self.text_fill,
                font="Proxima 9",
...
def simplify_buttons(data):
    ...
    data.buttons += [
        ...
        Button(
            ...
            18 / data.columns,
            2 / data.rows,
            remove_selected_songs_from_stage,
            3 / data.columns,
            0.95 / data.rows,
            red,
            red_hover,
        ...
        Sidebar(
            data,
            "Short Term Top Songs",
            1 - data.start_list,
            data.songs_top["short_term"],
        ),
        Sidebar(
            data,
            "Medium Term Top Songs",
            2 - data.start_list,
        ...
        ArtistSidebar(data, "YOUR TOP ARTISTS", 5 - data.start_list, data.top_artists),
        ArtistSidebar(
            data,
    ...
    for offset, playlist_id in enumerate(data.playlists):
        data.buttons += [
            Sidebar(
                data,
                truncate(
                    data.playlists[playlist_id]["name"], data.width / data.columns * 3
                ),
    ...
    if data.is_songs:
        data.buttons += [
            SongHeader(
                data,
                truncate("TITLE", data.width / data.columns * 3),
                0,
            ...
            SongHeader(
                data,
                truncate("ALBUM", data.width / data.columns * 3),
                2,
                order_songs_by_album,
            ),
            SongHeader(
                data,
                truncate("TIME", data.width / data.columns * 3),
                3,
                order_songs_by_time,
            ),
        ]
        ...
        data.buttons += [
            ArtistHeader(
                ...
                truncate("ARTIST", data.width / data.columns * 3),
                0,
                order_artists_by_name,
            ),
            ArtistHeader(
                data,
                truncate("TITLE", data.width / data.columns * 3),
                1,
                order_artists_by_title,
            ),
            ArtistHeader(
                data,
                truncate("ALBUM", data.width / data.columns * 3),
                2,
                order_artists_by_album,
            ),
            ArtistHeader(
                data,
                truncate("TIME", data.width / data.columns * 3),
                3,
                order_artists_by_time,
            ),
        ]
...
def simplify_mouse_moved(data):
    for song in data.viewing_songs:
        if song.is_within_bounds(data, data.mouse_moved_x, data.mouse_moved_y):
            song.hover()
        else:
            song.unhover()
    ...
    for artist in data.viewing_artists:
        if artist.is_within_bounds(data, data.mouse_moved_x, data.mouse_moved_y):
            artist.hover()
        else:
            artist.unhover()
...
def simplify_left_pressed(data):
    for button in data.buttons:
        if button.is_within_bounds(data, data.left_pressed_x, data.left_pressed_y):
            button.press(data)
        else:
            button.unpress(data)

    for i, song in enumerate(data.viewing_songs):
        if song.is_within_bounds(data, data.left_pressed_x, data.left_pressed_y):
            if data.shift:
                    ...
                    if i == j:
                        ...
                        if j < i:
                            for k in range(j, i + 1):
                                data.selected_ids.add(data.viewing_songs[k].get_id())
                        elif j > i:
            ...
            if data.shift:
                ...
                for j, song_j in enumerate(data.stage_songs):
                    if i == j:
                        ...
                        if j < i:
                            for k in range(j, i + 1):
                                data.selected_ids.add(data.stage_songs[k].get_id())
                        elif j > i:
                            for k in range(i, j + 1):
                                data.selected_ids.add(data.stage_songs[k].get_id())
                        break
    ...
    for i, artist in enumerate(data.viewing_artists):
        if artist.is_within_bounds(data, data.left_pressed_x, data.left_pressed_y):
            if data.shift:
                artist.press(data)
                ...
                < data.left_pressed_x
                < data.width / data.columns * 6
        ) and (not (data.ctrl or data.shift)):
            artist.unpress(data)
...
def lists_move_down(data):
    ...
    for i, sidebar_button in enumerate(sidebar_buttons):
        if data.ctrl:
            ...
            return
        if (i < (len(sidebar_buttons) - 1)) and sidebar_button.is_selected(data):
    ...
    for i, song in enumerate(data.stage_songs[::-1]):
        ...
        if (0 < song.get_row() < 40) and song.is_selected(data):
            if (song.get_row() == 39) and (index < len(data.stage_songs) - 1):
                ...
                refresh_view(data)
                return
            elif (song.get_row() < 39) and (index < len(data.stage_songs) - 1):
                data.selected_ids.clear()
                data.selected_ids.add(data.stage_songs[index + 1].get_id())
                refresh_view(data)
                return
...
def songs_move_down(data):
    ...
    for i, song in enumerate(data.viewing_songs[::-1]):
        index = len(data.viewing_songs) - i - 1
        if (0 < song.get_row() < 30) and song.is_selected(data):
            if (song.get_row() == 29) and (index < len(data.viewing_songs) - 1):
                data.selected_ids.clear()
                data.start_song += 1
                refresh_view(data)
                data.selected_ids.add(data.viewing_songs[index].get_id())
                refresh_view(data)
                return
            elif (song.get_row() < 29) and (index < len(data.viewing_songs) - 1):
                data.selected_ids.clear()
                data.selected_ids.add(data.viewing_songs[index + 1].get_id())
                refresh_view(data)
                return
...
def artists_move_down(data):
    ...
    for i, artist in enumerate(data.viewing_artists[::-1]):
        ...
        if (0 < artist.get_row() < 30) and artist.is_selected(data):
            if (artist.get_row() == 29) and (index < len(data.viewing_artists) - 1):
                data.selected_artists.clear()
                data.start_artist += 1
                refresh_view(data)
                data.viewing_artists[index].press(data)
                refresh_view(data)
                return
            elif (artist.get_row() < 29) and (index < len(data.viewing_artists) - 1):
                data.selected_artists.clear()
                data.viewing_artists[index + 1].press(data)
                refresh_view(data)
                return
...
def artist_songs_move_down(data):
    ...
    for i, song in enumerate(data.viewing_artist_songs[::-1]):
        index = len(data.viewing_artist_songs) - i - 1
        if (0 < song.get_row() < 30) and song.is_selected(data):
            if (song.get_row() == 29) and (index < len(data.viewing_artist_songs) - 1):
                data.selected_ids.clear()
                data.start_song += 1
                refresh_view(data)
                ...
                refresh_view(data)
                return
            elif (song.get_row() < 29) and (index < len(data.viewing_artist_songs) - 1):
                data.selected_ids.clear()
...
def simplify_redraw_all(canvas, data):
    canvas.create_rectangle(
        0, 0, data.width, data.height, fill=black_background, width=0
    )
    canvas.create_rectangle(
        data.width / data.columns * 3,
        0,
        data.width / data.columns * 16,
        data.height,
        fill=black_shadow,
    )
    for song in data.viewing_songs:
...
def analyze_buttons(data):
    ...
    data.buttons += [
        ...
        Sidebar(data, "YOUR CUSTOM PLAYLIST", 5 - data.start_list, data.stage_ids),
    ]
    for offset, playlist_id in enumerate(data.playlists):
        data.buttons += [
            Sidebar(
...
def create_playlist(data):
    ...
    results = sp.user_playlist_add_tracks(data.username, playlist_uri, data.song_ids)
...
def distribution_plot(data):
    ...
    sns.distplot(data.values, hist=False, color="g", kde_kws={"shade": True})

    f.suptitle(
        "%s of %s" % (data.parameter, data.selected_list),
        fontsize=14,
        fontweight="bold",
    )
